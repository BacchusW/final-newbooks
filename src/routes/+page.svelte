<script>
  let { data } = $props(); // Assuming data.transactions comes from your load function
	...
	// Create a filtered list that only keeps the first instance of each unique transaction ID
	let uniqueTransactions = $derived([
		...new Map(transactions.map((item) => [item.id, item])).values()
		// Note: Change 'id' to 'transaction_id' or whatever your primary key field is named
	]);
	...
	<section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
		...
		<div class="overflow-x-auto">
			<table class="w-full text-sm">
				...
				<tbody>
					{#each uniqueTransactions as t (t.id)}
